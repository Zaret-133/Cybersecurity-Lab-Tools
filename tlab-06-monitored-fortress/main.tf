# ==============================================================================
# DATA SOURCES (Provided in Starter Code)
# ==============================================================================
...
# ==============================================================================
# 1. THE PERIMETER (VPC, Subnet, Internet Gateway, Route Table)
# ==============================================================================
...
resource "aws_cloudwatch_log_group" "flow_logs_group" {
  ...
  tags = {
    Name = "titan-vpc-flow-logs-group"
  }
}
...
resource "aws_flow_log" "vpc_flow_log" {
  ...
  tags = {
    Name = "titan-vpc-flow-log"
  }
}


# ==============================================================================
# 3. ZERO TRUST COMPUTE (Security Group & EC2 Instance)
# ==============================================================================

# Security group with ZERO ingress rules (No inbound ports open)
resource "aws_security_group" "zero_trust_sg" {
  name        = "titan-zero-trust-sg"
  description = "Zero Inbound Security Group for SSM access"
  vpc_id      = aws_vpc.titan_vpc.id

  # Outbound traffic allowed so SSM Agent can reach AWS endpoints
  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
...
resource "aws_instance" "zero_trust_ec2" {
